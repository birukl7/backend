...
function GoogleCalendarBanner({ connected }: { connected: boolean }) {
    const { props } = usePage<{ flash?: { success?: string; error?: string } }>();
    const flash = props.flash;

    return (
        <div className={`flex items-center gap-3 rounded-2xl border px-4 py-3 ${connected ? "border-emerald-200 bg-emerald-50" : "border-blue-200 bg-blue-50"}`}>
            {/* Google icon */}
            ...
            <div className="flex-1 min-w-0">
                {flash?.success && <p className="text-[12px] text-emerald-700 font-medium">{flash.success}</p>}
                {flash?.error && <p className="text-[12px] text-red-600 font-medium">{flash.error}</p>}
                {!flash?.success && !flash?.error && (
                    connected
                        ? <p className="text-[12px] text-emerald-700 font-medium">Google Calendar connected — interviews sync automatically.</p>
                        : <p className="text-[12px] text-blue-700 font-medium">Connect Google Calendar to see your interviews there too.</p>
                )}
            </div>
            ...
            {connected ? (
                <button
                    onClick={() => {
                        if (confirm("Disconnect Google Calendar? Future interviews won't sync.")) {
                            router.post("/auth/google/calendar/disconnect", {}, { preserveScroll: true });
                        }
                    }}
                    className="shrink-0 text-[11px] font-semibold text-slate-500 hover:text-red-500 transition-colors underline underline-offset-2"
                >
                    Disconnect
                </button>
            ) : (
                <a
                    ...
                    className="shrink-0 inline-flex items-center gap-1.5 text-[12px] font-semibold text-blue-700 bg-white border border-blue-200 rounded-lg px-3 py-1.5 hover:bg-blue-50 transition-colors"
                >
                    Connect
                </a>
            )}
...
export default function InterviewsIndex({ interviews, google_calendar_connected }: Props) {
            ...
            <div className="max-w-5xl mx-auto px-6 py-8">
                ...
                <div className="flex gap-6 items-start">
                    ...
                    <div className="w-64 shrink-0 space-y-4">
                        ...
                        <GoogleCalendarBanner connected={google_calendar_connected} />
